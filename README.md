...
普通用户可以直接在 `[12] Bio 自动更新` 面板中选择内置模板，并在子面板里设置出生日期、固定 Bio、选择模板和预览输出。
...
同一个文件可以放多个模板，保存后进入 `[12] Bio 自动更新` -> `[4] 选择 Bio 模板` 即可在面板中看到并选择。
...
- `digit_style` / `max_length`：数字字体配置和 Telegram Bio 长度限制
