...
- 🔄 支持脚本自更新
- 📝 支持按出生日期每日自动更新 Bio
- 💤 Bio 功能开启后，每日 02:00-04:00 暂停动态 Last Name
- 🧹 支持一键卸载
...
- 每天 02:00-04:00 暂停动态 Last Name，并将 Last Name 显示为 `💤`
- 04:00 后自动恢复原有动态 Last Name
...
频率使用服务器本地时间对齐，不从服务启动时刻累计。Bio 功能的 `💤` 时段仍会在 02:00 开始、04:00 恢复。

选择 `[14] Emoji 时段` 可以添加、删除或清空规则：
...
测试覆盖配置迁移、Emoji 时段与排序、数字字体、Last Name/Bio 强制更新、天气缓存、凭证校验、Session 回滚和版本比较。
...
- 从 GitHub 拉取 `tg_panel.py`、`tg_daemon.py` 和 `requirements.txt`
- 下载到随机临时文件
- 检查 Python 语法
- 检查 `CURRENT_VERSION`
- 阻止低版本覆盖高版本
